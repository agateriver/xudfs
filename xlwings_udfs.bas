...
Attribute xxJoin.VB_Description = "将选定range内的字符串用sep连接起来"
...
Attribute xxRegexSub.VB_Description = "用正则表达式替换字符串"
...
Attribute xxSlice.VB_Description = "返回起始范围内的字串"
...
Attribute xxStrip.VB_Description = "返回去除首尾指定字符的字符串，默认去除首位全角空格、半角空格及换行"
